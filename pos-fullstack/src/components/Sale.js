...
function SaleForm() {
  ...
  const handleSaleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:3200/store/addsale', saleToSubmit);
    ...
    async function fetchProducts() {
      try {
        const response = await axios.get(
          `http://localhost:3200/store/Allproducts?store=${store || storeId}` // Use selected store or user's storeID
        );
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3200/store/Locations')
      .then((response) => {
